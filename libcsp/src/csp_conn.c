...
int csp_conn_print_table_str(char *str_buf, int str_size) {
	/* Display up to 6 connections */
	unsigned int start = (csp_conf.conn_max > 6) ? (csp_conf.conn_max - 6) : 0;

	for (unsigned int i = start; i < csp_conf.conn_max; i++) {
		csp_conn_t *conn = &arr_conn[i];
		char buf[240];
		//snprintf(buf, sizeof(buf), "[%02u %p] S:%u, %u -> %u, %u -> %u, sock: %p\r\n", i, conn, conn->state, conn->idin.src,
		//		conn->idin.dst, conn->idin.dport, conn->idin.sport, conn->socket);
		snprintf(buf, sizeof(buf), "[%02u] S:%u, %u->%u, %u->%u, s:%p\r\n",
				i, conn->state, conn->idin.src, conn->idin.dst, conn->idin.dport, conn->idin.sport, conn->socket);
